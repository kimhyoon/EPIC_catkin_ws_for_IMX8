...
int FastExplorationManager::planGoalPath(const Eigen::Vector3d &goal_pos, double goal_yaw) {
  ...
  for (auto& nbr : connection_candidates) {
    ...
    if (res == ParallelBubbleAstar::REACH_END) {
      ...
      else if (res == ParallelBubbleAstar::TIME_OUT) reason = "timeout (rejected)";
  ...
  bool search_success = planner_manager_->topo_graph_->graphSearch(
      start_node, goal_node, topo_path, 0.1);
...
void FastExplorationManager::simplifyGlobalTour() {
  ...
  simplified_tour.push_back(ed_->global_tour_[0]);

  const double MAX_SEGMENT_LENGTH = ep_->max_segment_length_;
  int collision_checks = 0;
  int anchor_idx = 0;

  while (anchor_idx < (int)ed_->global_tour_.size() - 1) {
    int furthest_visible = anchor_idx + 1;

    for (int test_idx = anchor_idx + 2; test_idx < (int)ed_->global_tour_.size(); ++test_idx) {
    ...
    if (furthest_visible < (int)ed_->global_tour_.size() - 1) {
